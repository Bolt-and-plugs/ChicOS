...
void retrieve_addr(const user *u, char *dest, const size_t addr_sz) {
#ifdef __linux__
...
bool decrypt(const char *stored_hash, const char *provided_password) {
  if (crypto_pwhash_str_verify(stored_hash, provided_password,
                               strlen(provided_password)) != 0) {
    return false;
  }
  return true;
}
...
user *read_login_data(const user *u) {
  ...
  if (strcmp(local->username, u->username) == 0 &&
    ...
    log_event("LOGIN_FAILURE", local->password);
  }
  ...
  if (decrypt(local->password, u->password) == true)
    log_event("CRIPTOGRAFIA", local->password);
